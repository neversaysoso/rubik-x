...
var mui = function(a, b) {
	...
	}, "function" == typeof define && define.amd && define("rbk", [], function() {
		return i
	}), i
}(document);
! function(a, b) {
	...
	a(function() {
		if (a.os.ios) {
			var c = "rbk-focusin",
				d = "rbk-bar-tab",
				e = "rbk-bar-footer",
				f = "rbk-bar-footer-secondary",
				g = "rbk-bar-footer-secondary-tab";
...
function(a) {
	a.namespace = "rbk", a.classNamePrefix = a.namespace + "-", a.classSelectorPrefix = "." + a.classNamePrefix, a.className = function(b) {
...
function(a) {
	var b = 1,
		...
		h = function(a) {
			return a._mid || (a._mid = b++)
		},
		i = {},
		j = function(b, d, e, f) {
			return function(e) {
				for (var f = c[b._mid][d], h = [], i = e.target, j = {}; i && i !== document && i !== b && (!~["click", "tap", "doubletap", "longtap", "hold"].indexOf(d) || !i.disabled && !i.classList.contains("rbk-disabled")); i = i.parentNode) {
					var k = {};
	...
	a.fn.on = function(b, d, e) {
		return this.each(function() {
			...
			var g = !1,
				k = c[f._mid] || (c[f._mid] = {}),
				l = k[b] || (k[b] = {});
			a.isEmptyObject(l) && (g = !0);
			var m = l[d] || (l[d] = []);
			m.push(e), g && (i[h(f)] = j(f, b, d, e), f.addEventListener(b, i[h(f)]), "tap" === b && f.addEventListener("click", function(a) {
				if (a.target) {
...
function(a, b) {
	...
	b.addEventListener("scroll", function() {
		a.isScrolling = !0, j && clearTimeout(j), j = setTimeout(function() {
			a.isScrolling = !1
		}, 250)
	})
}(mui, window),
...
function(a, b) {
	var c = function(c, d) {
		switch (c.type) {
			case a.EVENT_MOVE:
				d.direction && (d.lockDirection && d.startDirection && d.startDirection && d.startDirection !== d.direction && ("up" === d.startDirection || "down" === d.startDirection ? d.direction = d.deltaY < 0 ? "up" : "down" : d.direction = d.deltaX < 0 ? "left" : "right"), d.drag || (d.drag = !0, a.trigger(c.target, b + "start", d)), a.trigger(c.target, b, d), a.trigger(c.target, b + d.direction, d));
				break;
			case a.EVENT_END:
			case a.EVENT_CANCEL:
	...
	var c = function(c, d) {
		if (c.type === a.EVENT_END) {
			...
			if (d.distance < e.tapMaxDistance && d.touchTime < e.tapMaxTime) {
				if (a.options.gestureConfig.doubletap && d.lastTarget && d.lastTarget === c.target && d.lastTapTime && d.startTime - d.lastTapTime < e.tapMaxInterval) return a.trigger(c.target, "doubletap", d), d.lastTapTime = a.now(), void(d.lastTarget = c.target);
				a.trigger(c.target, b, d), d.lastTapTime = a.now(), d.lastTarget = c.target
			}
		}
	};
	a.registerGesture({
...
function(a) {
	...
	a.init = function(d) {
		...
		}), b.add("rbk-ios")) : a.os.android && (c.push({
			os: "android",
			version: a.os.version
		}), b.add("rbk-android")), a.os.wechat && (c.push({
			os: "wechat",
			version: a.os.wechat.version
		}), b.add("rbk-wechat")), c.length && a.each(c, function(c, d) {
			var e = "";
...
function(a, b) {
	a.registerBack = function(b) {
		...
		c && c.classList.contains("rbk-action-back") && a.back()
...
function(a, b) {
	a.os.plus && a.os.android && a.registerBack({
		...
		handle: function() {
			...
			var b = document.querySelector(".rbk-off-canvas-wrap.rbk-active");
		...
		var c = document.querySelector(".rbk-action-menu");
					...
					1 === g.length && (a.os.plus && a.os.android ? a.plusReady(function() {
						...
						if (d) {
							...
							if (h) {
								...
								j.down = a.extend({}, c.down), j.down.callback = "_CALLBACK", h.evalJS("mui&&mui(document.querySelector('.rbk-content')).pullRefresh('" + JSON.stringify(j) + "')")
							}
						}
					}) : g.pullRefresh(c))
				}
...
function(a, b, c) {
	var d = "rbk-pull-top-pocket",
		e = "rbk-pull-bottom-pocket",
		f = "rbk-pull",
		g = "rbk-pull-loading",
		h = "rbk-pull-caption",
		i = "rbk-icon",
		...
		p = g + " " + i + " " + k,
		q = g + " " + i + " " + j,
		r = ['<div class="' + f + '">', '<div class="{icon}"></div>', '<div class="' + h + '">{contentrefresh}</div>', "</div>"].join(""),
		s = {
			init: function(b, c) {
			...
			_initPulldownRefresh: function() {
				this.pulldown = !0, this.pullPocket = this.topPocket, this.pullPocket.classList.add(l), this.pullPocket.classList.add(n), this.pullCaption = this.topCaption, this.pullLoading = this.topLoading
			},
			_initPullupRefresh: function() {
				this.pulldown = !1, this.pullPocket = this.bottomPocket, this.pullPocket.classList.add(l), this.pullPocket.classList.add(n), this.pullCaption = this.bottomCaption, this.pullLoading = this.bottomLoading
			},
			_initPocket: function() {
				var a = this.options;
				a.down && a.down.hasOwnProperty("callback") && (this.topPocket = this.scroller.querySelector("." + d), this.topPocket || (this.topPocket = this._createPocket(d, a.down, p), this.wrapper.insertBefore(this.topPocket, this.wrapper.firstChild)), this.topLoading = this.topPocket.querySelector("." + g), this.topCaption = this.topPocket.querySelector("." + h)), a.up && a.up.hasOwnProperty("callback") && (this.bottomPocket = this.scroller.querySelector("." + e), this.bottomPocket || (this.bottomPocket = this._createPocket(e, a.up, q), this.scroller.appendChild(this.bottomPocket)), this.bottomLoading = this.bottomPocket.querySelector("." + g), this.bottomCaption = this.bottomPocket.querySelector("." + h), this.wrapper.addEventListener("scrollbottom", this))
			},
			_createPocket: function(a, c, d) {
				var e = b.createElement("div");
				return e.className = a, e.innerHTML = r.replace("{contentrefresh}", c.contentrefresh).replace("{icon}", d), e
			},
			_resetPullDownLoading: function() {
				var a = this.pullLoading;
				a && (this.pullCaption.innerHTML = this.options.down.contentdown,
					a.style.webkitTransition = "", a.style.webkitTransform = "", a.style.webkitAnimation = "", a.className = p)
			},
				...
				if (!this.loading) {
					...
					if (d)
						if (b) {
							var h = this;
							setTimeout(function() {
								e.innerHTML = h.lastTitle = a, g ? f.className = p : f.className = q, f.style.webkitAnimation = "", f.style.webkitTransition = "", f.style.webkitTransform = ""
							}, 100)
						} else a !== this.lastTitle && (e.innerHTML = a, g ? a === c.down.contentrefresh ? (f.className = q, f.style.webkitAnimation = "spinner-spin 1s step-end infinite") : a === c.down.contentover ? (f.className = o, f.style.webkitTransition = "-webkit-transform 0.3s ease-in", f.style.webkitTransform = "rotate(180deg)") : a === c.down.contentdown && (f.className = p, f.style.webkitTransition = "-webkit-transform 0.3s ease-in", f.style.webkitTransform = "rotate(0deg)") : a === c.up.contentrefresh ? f.className = q + " " + n : f.className = q + " " + m, this.lastTitle = a)
				}
			}
		};
	a.PullRefresh = s
	...
	var e = "rbk-scrollbar",
		...
		k = a.Class.extend({
			...
			_initEvent: function() {
				...
				var c = this.wrapper.querySelector(".rbk-segmented-control");
			...
			setTranslate: function(a, b) {
				if (this.x = a, this.y = b, this.scrollerStyle.webkitTransform = this._getTranslateStr(a, b), this.parallaxElement && this.options.scrollY) {
					...
					d > 1 ? (this.parallaxImgStyle.opacity = 1 - c / 100 * this.options.parallaxRatio, this.parallaxStyle.webkitTransform = this._getTranslateStr(0, -c) + " scale(" + d + "," + d + ")") : (this.parallaxImgStyle.opacity = 1, this.parallaxStyle.webkitTransform = this._getTranslateStr(0, -1) + " scale(1,1)")
				}
				if (this.indicators)
	...
	l.prototype = {
		...
		return this.each(function() {
			...
			if (f) d = a.data[f];
				...
				f = ++a.uuid;
				var g = a.extend({}, b);
				e.classList.contains("rbk-segmented-control") && (g = a.extend(g, {
					scrollY: !1,
					scrollX: !0,
					indicators: !1,
...
function(a, b, c, d) {
	var e = "rbk-visibility",
		...
		g = a.Scroll.extend(a.extend({
			...
			refresh: function(a) {
				a && this.finished && (this.pulldown !== !1 && this._initPullupRefresh(), this.bottomPocket.classList.remove(f), this._setCaption(this.options.up.contentdown), this.wrapper.addEventListener("scrollbottom", this), this.finished = !1), this._super()
			}
		}, a.PullRefresh));
	a.fn.pullRefresh = function(b) {
		if (1 === this.length) {
			...
			return e ? d = a.data[e] : (e = ++a.uuid, a.data[e] = d = new g(c, b), c.setAttribute("data-pullrefresh", e)), b.up && b.up.auto && d.pullupLoading(), d
		}
	}
}(mui, window, document),
function(a, b) {
	var c = "rbk-slider",
		d = "rbk-slider-group",
		e = "rbk-slider-loop",
		f = "rbk-slider-indicator",
		g = "rbk-action-previous",
		h = "rbk-action-next",
		i = "rbk-slider-item",
		j = "rbk-active",
		k = "." + i,
		l = "." + f,
		m = ".rbk-slider-progress-bar",
		n = a.Scroll.extend({
			init: function(b, c) {
				this._super(b, a.extend(!0, {
					interval: 0,
				...
				b._super(), b.wrapper.addEventListener("swiperight", a.stopPropagation), b.wrapper.addEventListener("scrollend", function() {
					...
					if (c.target === b.wrapper) {
						...
						var e = b.scroller.querySelectorAll(k),
							f = d.slideNumber;
						if (b.loop && (f += 1), !b.wrapper.classList.contains("rbk-segmented-control"))
							for (var g = 0, h = e.length; h > g; g++) {
								var i = e[g];
								i.parentNode === b.scroller && (g === f ? i.classList.add(j) : i.classList.remove(j))
							}
						var l = b.wrapper.querySelector(".rbk-slider-indicator");
						if (l) {
							l.getAttribute("data-scroll") && a(l).scroll().gotoPage(d.slideNumber);
							var m = l.querySelectorAll(".rbk-indicator");
							if (m.length > 0)
								for (var g = 0, h = m.length; h > g; g++) m[g].classList[g === d.slideNumber ? "add" : "remove"](j);
							else {
								var n = l.querySelector(".rbk-number span");
								if (n) n.innerText = d.slideNumber + 1;
								else
									for (var o = b.wrapper.querySelectorAll(".rbk-control-item"), g = 0, h = o.length; h > g; g++) o[g].classList[g === d.slideNumber ? "add" : "remove"](j)
							}
						}
						c.stopPropagation()
					}
	...
	a.fn.slider = function(b) {
		...
		a(".rbk-slider").slider(), a(".rbk-scroll-wrapper.rbk-slider-indicator.rbk-segmented-control").scroll({
			scrollY: !1,
			scrollX: !0,
			indicators: !1,
			snap: ".rbk-control-item"
		...
		var c = "rbk-plus-pullrefresh",
			...
			g = a.Class.extend({
				...
				endPulldownToRefresh: function() {
					...
					a.parent().evalJS("mui&&mui(document.querySelector('.rbk-content')).pullRefresh('" + JSON.stringify({
						webviewId: a.id
				...
				endPullupToRefresh: function(a) {
					var b = this;
					b.pullLoading && (b.pullLoading.classList.remove(d), b.pullLoading.classList.add(e), b.isLoading = !1, a ? (b.finished = !0, b.pullCaption.innerHTML = b.options.up.contentnomore, window.removeEventListener("dragup", b)) : b.pullCaption.innerHTML = b.options.up.contentdown)
				},
				scrollTo: function(b, c, d) {
					a.scrollTo(b, c, d)
				},
				refresh: function(a) {
					a && this.finished && (this.pulldown !== !1 && this._initPullupRefresh(), this.bottomPocket.classList.remove(e), this.pullCaption.innerHTML = this.options.up.contentdown, window.addEventListener("dragup", this), this.finished = !1)
				}
			}, a.PullRefresh));
		a.fn.pullRefresh = function(d) {
			var e;
			0 === this.length ? (e = b.createElement("div"), e.className = "rbk-content", b.body.appendChild(e)) : e = this[0], d = d || {}, "string" == typeof d && (d = a.parseJSON(d)), !d.webviewId && (d.webviewId = plus.webview.currentWebview().id || plus.webview.currentWebview().getURL());
			var f = null,
				h = d.webviewId && d.webviewId.replace(/\//g, "_"),
				i = e.getAttribute("data-pullrefresh-plus-" + h);
			return i ? f = a.data[i] : (i = ++a.uuid, e.setAttribute("data-pullrefresh-plus-" + h, i), b.body.classList.add(c), a.data[i] = f = new g(e, d)), d.up && d.up.auto && f.pullupLoading(), f
		}
	}
}(mui, document),
function(a, b, c, d) {
	var e = "rbk-off-canvas-left",
		f = "rbk-off-canvas-right",
		g = "rbk-off-canvas-backdrop",
		h = "rbk-off-canvas-wrap",
		i = "rbk-slide-in",
		j = "rbk-active",
		k = "rbk-transitioning",
		l = ".rbk-inner-wrap",
		m = a.Class.extend({
			...
			initEvent: function() {
				...
				}), this.classList.contains("rbk-draggable") && (this.wrapper.addEventListener("touchstart", this), this.wrapper.addEventListener("drag", this), this.wrapper.addEventListener("dragend", this)), this.wrapper.addEventListener("webkitTransitionEnd", this)
			},
			...
			close: function() {
				this._initOffCanvasVisible(), this.slideIn && (this.scroller = this.wrapper.querySelector("." + f + "." + j) || this.wrapper.querySelector("." + e + "." + j)), this.scroller && (this.scroller.classList.add(k), this.openPercentage(0))
			},
			show: function(a) {
				return this._initOffCanvasVisible(), this.isShown(a) ? !1 : (a || (a = this.wrapper.querySelector("." + f) ? "right" : "left"), this.slideIn && (this.scroller = "right" === a ? this.offCanvasRight : this.offCanvasLeft), this.scroller && (this.scroller.classList.add(k), this.openPercentage("left" === a ? 100 : -100)), !0)
			},
			toggle: function(a) {
				var b = a;
	...
	a.registerTarget({
		...
		if (a.targets.offcanvas)
			for (var d = b.target; d && d !== c; d = d.parentNode)
				if ("A" === d.tagName && d.hash && d.hash === "#" + a.targets.offcanvas.id) {
					b.detail.gesture.preventDefault(), a(a.targets._container).offCanvas().toggle(a.targets.offcanvas), a.targets.offcanvas = a.targets._container = null;
					break
				}
		...
		a(".rbk-off-canvas-wrap").offCanvas()
	})
...
function(a, b, c, d) {
	...
	a.registerTarget({
		...
		a.targets.modal && (b.detail.gesture.preventDefault(), a.targets.modal.classList.toggle("rbk-active"))
	})
}(mui, window, document, "modal"),
function(a, b, c, d) {
	var e = "rbk-popover",
		f = "rbk-popover-arrow",
		g = "rbk-popover-action",
		h = "rbk-backdrop",
		i = "rbk-bar-popover",
		j = "rbk-bar-backdrop",
		k = "rbk-backdrop-action",
		l = "rbk-active",
		m = "rbk-bottom",
		n = function(b, d) {
			if ("A" === d.tagName && d.hash) {
	...
	var t = function(a, b) {
			...
			var d = c.querySelector(".rbk-popover.rbk-active");
...
function(a, b, c, d, e) {
	var f = "rbk-control-item",
		g = "rbk-segmented-control",
		h = "rbk-control-content",
		i = "rbk-bar-tab",
		j = "rbk-tab-item",
		k = function(a, b) {
	...
	a.registerTarget({
		...
		if (e) {
			for (var k, l, m, n = "rbk-active", o = "." + n, p = e.parentNode; p && p !== c; p = p.parentNode) {
...
function(a, b, c) {
	...
	var e, f, g = "rbk-active",
		h = "rbk-selected",
		i = "rbk-grid-view",
		j = "rbk-table-view-radio",
		k = "rbk-table-view-cell",
		l = "rbk-collapse-content",
		m = "rbk-disabled",
		n = "rbk-switch",
		o = "rbk-btn",
		p = "rbk-slider-handle",
			...
			if (g.classList) {
				...
				if (p.contains(k)) {
					...
					s && (d(e), b.stopPropagation()), h || (s ? (B && B.cancel(), B = a.later(function() {
						C(!0)
					}, 100)) : e.querySelector("input") || e.querySelector(x) || e.querySelector("." + n) || C(!0));
					break
				}
			}
	...
	var G = function() {
		var a = e.classList;
		if (a.contains("rbk-radio")) {
			var b = e.querySelector("input[type=radio]");
			b && (b.checked = !b.checked)
		} else if (a.contains("rbk-checkbox")) {
			var b = e.querySelector("input[type=checkbox]");
			b && (b.checked = !b.checked)
		}
	};
	b.addEventListener(a.EVENT_CLICK, function(a) {
		e && e.classList.contains("rbk-collapse") && a.preventDefault()
	}), b.addEventListener("doubletap", function(a) {
		e && G()
	}), b.addEventListener("tap", function(b) {
		if (e) {
			var c = b.target && b.target.type;
			if ("radio" === c || "checkbox" === c) return void G();
			var d = !1,
				f = e.classList,
				i = e.parentNode;
			if (i && i.classList.contains(j)) {
				if (f.contains(h)) return;
				var k = i.querySelector("li" + w);
				return k && k.classList.remove(h), f.add(h), void a.trigger(e, "selected", {
					el: e
				})
			}
			if (f.contains("rbk-collapse") && !e.parentNode.classList.contains("rbk-unfold")) {
				if (b.detail.gesture.preventDefault(), !f.contains(g)) {
					var l = e.parentNode.querySelector(".rbk-collapse.rbk-active");
					l && l.classList.remove(g), d = !0
				}
				f.toggle(g), d && a.trigger(e, "expand")
			} else G()
		}
	})
}(mui, window, document),
...
function(a, b) {
	a.toast = function(b) {
		if (a.os.plus) plus.nativeUI.toast(b, {
			...
			var c = document.createElement("div");
			c.classList.add("rbk-toast-container"), c.innerHTML = '<div class="rbk-toast-message">' + b + "</div>", document.body.appendChild(c), setTimeout(function() {
...
function(a, b, c) {
	...
	r.prototype.init = function() {
		...
		if (!f) {
			...
			f.className = b, b === this.searchActionClass && (f.innerHTML = '<span class="' + d + " " + g + '"></span>' + this.element.getAttribute("placeholder"), this.element.setAttribute("placeholder", ""), this.element.value.trim() && a.classList.add("rbk-active")), a.insertBefore(f, this.element.nextSibling)
		}
		return f
		...
		if (this.sliderActionClass) {
			...
			this.searchActionClass && (b.addEventListener("focus", function() {
				b.parentNode.classList.add("rbk-active")
			}), b.addEventListener("blur", function() {
				b.value.trim() || b.parentNode.classList.remove("rbk-active")
			}))
		}
		...
		this.each(function() {
			var b = [],
				c = q(this.parentNode);
			if ("range" === this.type && c.classList.contains("rbk-input-range")) b.push("slider");
			else {
				var d = this.classList;
				d.contains("rbk-input-clear") && b.push("clear"), d.contains("rbk-input-speech") && b.push("speech"), "search" === this.type && c.classList.contains("rbk-search") && b.push("search")
			}
...
function(a) {
	var b = "ontouchstart" in document,
		...
		d = "change",
		e = "rbk-numbox",
		f = "rbk-numbox-btn-plus",
		g = "rbk-numbox-btn-minus",
		h = "rbk-numbox-input",
